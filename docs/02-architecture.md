...
> 基于 [01-original-analysis.md](./01-original-analysis.md) 的分析结果  
> 本文档定义三种语言实现必须遵守的统一架构规范
...
- 使用真实 API Key 跑一个简单的编码任务（读文件→修改→验证）
- 验证会话持久化后可正确恢复上下文
...
```
docs/
├── 01-original-analysis.md   # 原始项目深度分析（本文的基础）
├── 02-architecture.md        # 本文：统一架构设计方案
├── 03-api-reference.md       # 各层接口完整参考（待编写）
└── 04-session-format.md      # 会话文件格式详细说明（待编写）

agent-python/TASK.md          # Python 实现任务书
agent-go/TASK.md              # Go 实现任务书
agent-rust/TASK.md            # Rust 实现任务书
```
